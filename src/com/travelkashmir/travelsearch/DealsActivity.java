...
public class DealsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list_view.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int pos,
					long arg3) {
				Intent intent=new Intent(DealsActivity.this,DetailsActivity.class);
				intent.putExtra("Deals", arrayJsonList.get(pos).toString());
				intent.putExtra("heading", "Deal");
	...
	public class GetDeals extends AsyncTask<String, Void, String> {
		...
		//public static final String serverUrl = Constant.serverNewsUrl+"GetAllPets&userid=1";
		...
		protected String doInBackground(String... params) {{
			...
			case Constant.DEALS:
				serverUrl = Constant.serverUrl+"GetDeals";
				jsonSubString="deal";
				break;}
				...
				try {
					...
					if (jsonArray != null) {
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							jsonObject = jsonObject.getJSONObject(jsonSubString);
		...
		public void run() {
			...
			for (int i = 0; i < arrayJsonList.size(); i++) {
				...
				try {
					...
					hashImages.put(arrayJsonList.get(i).getString("dealid"), bitmapImage);
